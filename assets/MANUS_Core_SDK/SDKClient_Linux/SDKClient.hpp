...
protected:
	...
	// ZMQ: high-frequency skeleton stream & lower-frequency ergonomics stream
	zmq::context_t ctx;          // shared context

	// skeleton pose output (352 floats / 176 floats)
	zmq::socket_t sock_skel;

	// ergonomics output (40 floats)
	zmq::socket_t sock_ergo;

	// buffer for snprintf (increase size to avoid overflow)
	char char_buf[4096];
